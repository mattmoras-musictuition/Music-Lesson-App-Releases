...
import { House, Calendar, CalendarDays, LayoutGrid, ClipboardCheck, GraduationCap, NotebookPen, Clock, Palette, Building2, Guitar, BookUser, Library, Settings, Piano, Music, MessageSquare, Lightbulb, Receipt, ChevronDown, ChevronUp } from "lucide-react";
...
import { ConcertsManager } from "./pages/ConcertsManager";
...
export default function MusicTimetableApp() {
  ...
  // Temporary-lanes session 2 — one-week-only lanes on a (school, day).
  // Empty until session 3's right-click "Add Teacher" UI lands.
  const [temporaryLanes, setTemporaryLanes] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      // ── Temporary lanes: Supabase only (no localStorage cache — week-keyed lane data) ──
      let tl;
      try {
        tl = await loadTemporaryLanesFromSupabase();
      } catch (err) {
        logError("Failed to load temporary_lanes from Supabase", err.message);
        tl = [];
      }
  ...
  const [ttAckedConstraints, setTtAckedConstraints] = React.useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem("ms_ttAckedConstraints_v1") || "[]")); }
    catch { return new Set(); }
  });
  const [weeklyConstraintWarnings, setWeeklyConstraintWarnings] = React.useState({});
  const [weeklyAckedConstraints, setWeeklyAckedConstraints] = React.useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem("ms_weeklyAckedConstraints_v1") || "[]")); }
    catch { return new Set(); }
  });
  // Persist standing constraint acknowledgements across app restart.
  React.useEffect(() => {
    try { localStorage.setItem("ms_ttAckedConstraints_v1", JSON.stringify([...ttAckedConstraints])); } catch {}
  }, [ttAckedConstraints]);
  React.useEffect(() => {
    try { localStorage.setItem("ms_weeklyAckedConstraints_v1", JSON.stringify([...weeklyAckedConstraints])); } catch {}
  }, [weeklyAckedConstraints]);
  ...
  // to that (school, week). No day-column highlight on landing (WTT's
  // selectedDays is local state and all weekday columns are visible).
  const handleJumpToWeekly = (school, weekOffset) => {
    setSharedSchool(school.id);
    setWeeklyViewState(prev => ({ ...prev, selectedSchool: school.id, weekOffset }));
    setPage("weekly");
  };
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "'DM Sans', sans-serif", background: colors.bg, color: colors.text, overflow: "hidden" }}>
      ...
      <div className="no-print" ref={sidebarRefCb} style={{ width: 240, background: colors.sidebar, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <nav style={{ flex: 1, padding: "12px 8px", overflowY: "auto", overflowX: "hidden" }}>
          {[
            ...
            { id: "concerts", icon: <Music size={16} />, label: "Concerts" },
        ...
        */}
        {teachers.length > 0 && (() => {
          ...
          // Pre-compute derived per-teacher data so both render paths share
          // the same online/offline determination and colour.
          const rows = teachers.map(t => {
            ...
            return { t, isOnline, presenceEntry, effectiveLastSeenIso, colour };
          });
          // Chevron lives in the bottom row of the panel and stays anchored
          // there in both states. Panel grows upward into the scrollable nav
          // above when expanded, so the bottom edge of the roster (and the
          // chevron with it) holds the same screen position. Glyph direction
          // reflects the action: ChevronUp when collapsed (panel will grow
          // up), ChevronDown when expanded (panel will shrink down).
          const chevronBtn = (
            <button
              type="button"
              onClick={() => setRosterExpanded(v => !v)}
              title={rosterExpanded ? "Collapse roster" : "Expand roster"}
              style={{ marginLeft: "auto", background: "none", border: "none", padding: 0, cursor: "pointer", color: "rgba(255,255,255,0.55)", display: "inline-flex", alignItems: "center", justifyContent: "center", lineHeight: 1, flexShrink: 0 }}>
              {rosterExpanded ? <ChevronDown size={14} /> : <ChevronUp size={14} />}
            </button>
          );
          return (
            <div style={{ padding: "8px 12px", borderTop: "1px solid rgba(255,255,255,0.08)", flexShrink: 0, display: "flex", flexDirection: "column" }}>
              {rosterExpanded && rows.map(({ t, isOnline, presenceEntry, effectiveLastSeenIso, colour }) => {
              ...
              {/* Bottom-anchored row: hosts the two-letter strip when
                  collapsed, just the chevron when expanded. Either way the
                  chevron sits at the bottom-right of the panel and keeps
                  its screen position across the toggle. */}
              <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 7, lineHeight: 1.3 }}>
                {!rosterExpanded && rows.map(({ t, colour }) => {
                  // Two-letter initial: first letter of first + last name
                  // token, or first two letters of a single-token name.
                  // Fallback "?" for empty/whitespace-only records so the
                  // gap is visible.
      ...
      <div ref={mainScrollRef} data-printarea="true" style={{ flex: 1, overflow: "auto", position: "relative" }}
        ...
        <div style={{ padding: "28px 36px", maxWidth: 1200 }}>
          ...
          {page === "concerts" && <ConcertsManager schools={schools} students={students} teachers={teachers} bands={bands} notify={notify} goBack={goBack} goForward={goForward} historyCursor={historyCursor} pageHistory={pageHistory} />}
