...
import { Library, FileText, Link, Plus, X, Check, Pencil, Trash2, Copy, AlertTriangle, Clock, Building2, Guitar, Eye, Upload, Download as DownloadIcon, Loader } from "lucide-react";
...
export function DocumentsResourcesManager({ resources, setResources, documents, setDocuments, schools, teachers, notify, resetKey, viewState, setViewState, goBack, goForward, historyCursor, pageHistory }) {
  ...
  const [rCatFilter,setRCatFilter]= useState("");
  ...
  const filteredResources = useMemo(() => resources.filter(r => {
    ...
    if (rCatFilter && r.category !== rCatFilter) return false;
    if (rSearch) {
      const q = rSearch.toLowerCase();
      return (r.label||"").toLowerCase().includes(q) || (r.description||"").toLowerCase().includes(q) || (r.url||"").toLowerCase().includes(q);
    }
    return true;
  }), [resources, rSearch, rCatFilter]);

  // Session 97: merged option lists used by both the filter dropdown and the
  // inline-edit combobox. Any custom category Matt types is preserved in the
  // documents/resources data, so it'll show up here next time.
  const resourceCategoryOptions = useMemo(
    () => _mergeOptions(RESOURCE_CATEGORIES, resources, "category"),
    [resources]
  );
        ...
        <div>
          <Card style={{ marginBottom: 16, padding: 14 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <select value={rCatFilter} onChange={e => setRCatFilter(e.target.value)} style={{ padding: "8px 12px", border: "1px solid " + colors.inputBorder, borderRadius: 8, fontSize: 13, fontFamily: "inherit", background: colors.cardBg, color: colors.text }}>
                <option value="">All Categories</option>
                {resourceCategoryOptions.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
          </Card>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {filteredResources.map(r => {
                    const isEditing = rEditId === r.id;
                    return (
                      <tr key={r.id}
                        style={{ background: isEditing ? colors.blueLight : (rHovered === r.id ? colors.blueLight : colors.cardBg), borderBottom: "1px solid " + colors.borderLight }}
                        onMouseEnter={() => setRHovered(r.id)} onMouseLeave={() => setRHovered(null)}>
                        <td style={{ padding: "8px 12px", fontWeight: 600 }}>
                          {isEditing
                            ? <input autoFocus value={rEditForm.label} onChange={e => setREditForm(f => ({ ...f, label: e.target.value }))} onKeyDown={e => { if (e.key === "Enter") saveResource(); if (e.key === "Escape") cancelResource(); }} placeholder="e.g. Essential Music Theory Book 1" style={inputStyle} />
                            : r.label || <span style={{ color: colors.textMuted, fontStyle: "italic" }}>—</span>}
                        </td>
                        <td style={{ padding: "8px 12px" }}>
                          {isEditing
                            ? <>
                                <input list={`resCatList-${r.id}`} value={rEditForm.category} onChange={e => setREditForm(f => ({ ...f, category: e.target.value }))} placeholder="Type or pick…" onKeyDown={e => { if (e.key === "Enter") saveResource(); if (e.key === "Escape") cancelResource(); }} style={{ padding: "5px 8px", border: "1px solid " + colors.inputBorder, borderRadius: 6, fontSize: 13, fontFamily: "inherit", background: colors.cardBg, color: colors.text, width: 140 }} />
                                <datalist id={`resCatList-${r.id}`}>
                                  {resourceCategoryOptions.map(c => <option key={c} value={c} />)}
                                </datalist>
                              </>
                            : r.category
                        ...
                        <td style={{ padding: "8px 12px", color: colors.textLight }}>
                          {isEditing
                            ? <input value={rEditForm.description} onChange={e => setREditForm(f => ({ ...f, description: e.target.value }))} onKeyDown={e => { if (e.key === "Enter") saveResource(); if (e.key === "Escape") cancelResource(); }} placeholder="Brief description" style={inputStyle} />
                          ...
                          {isEditing ? (
                            ...
                            rEditForm.file_url ? (
                              ...
                              <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                                <input value={rEditForm.url} onChange={e => setREditForm(f => ({ ...f, url: e.target.value }))} onKeyDown={e => { if (e.key === "Enter") saveResource(); if (e.key === "Escape") cancelResource(); }} placeholder="https://…" style={{ ...inputStyle, flex: 1 }} />
                                <button onClick={() => pickAndUploadFile(BUCKET_RESOURCES, rEditId, (patch) => setREditForm(f => ({ ...f, ...patch })))}
                                  disabled={uploadingFor === rEditId}
                                  title="Upload a file instead"
                                  style={{ border: `1px solid ${colors.inputBorder}`, background: colors.cardBg, borderRadius: 6, padding: "5px 7px", cursor: uploadingFor === rEditId ? "wait" : "pointer", display: "inline-flex", alignItems: "center", color: colors.textLight }}>
                                  {uploadingFor === rEditId ? <Loader size={12} className="spin" /> : <Upload size={12} />}
                                </button>
                              </div>
                            )
                        ...
                        <td style={{ padding: "8px 12px" }}>
                          <div style={{ display: "flex", gap: 4, justifyContent: "flex-end", alignItems: "center" }}>
                            {isEditing ? (
                              <>
                                <button onClick={saveResource} title="Save" style={{ border: "none", background: colors.success, color: "#fff", borderRadius: 6, padding: "4px 8px", cursor: "pointer", display: "inline-flex", alignItems: "center" }}><Check size={14} /></button>
                                <button onClick={cancelResource} title="Cancel" style={{ border: "1px solid " + colors.border, background: colors.cardBg, color: colors.textMuted, borderRadius: 6, padding: "4px 8px", cursor: "pointer", display: "inline-flex", alignItems: "center" }}><X size={14} /></button>
                              </>
                            ) : (
                              <>
