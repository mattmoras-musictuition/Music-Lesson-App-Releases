...
import { colors, DAYS, SLOT_TYPES, SLOT_TYPE_LABELS } from "../constants";
        ...
        <Card>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {DAYS.map(d => (
                <button key={d} onClick={() => toggleDay(d)} style={{ padding: "6px 14px", borderRadius: 6, fontSize: 13, fontFamily: "inherit", cursor: "pointer", border: `1px solid ${form.days.includes(d) ? colors.accent : colors.border}`, background: form.days.includes(d) ? colors.accentLight : colors.white, color: form.days.includes(d) ? colors.accentDark : colors.textLight, fontWeight: 500 }}>
                  {d.slice(0, 3)}
                </button>
              ))}
            </div>
          ...
          <div style={{ fontSize: 12, color: colors.textMuted, marginTop: -8, marginBottom: 14, paddingLeft: 2 }}>
            💡 Specialist class times are managed in the "Specialist Classes" section — this controls whether the scheduler can pull students out of those classes.
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 12, color: colors.textMuted, marginBottom: 10, paddingLeft: 2 }}>☕ Times when no teacher may have lessons at this school. These override individual teacher breaks set in the Teachers tab.</div>
                    ...
                    {slotGen.blocks.map((block, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", padding: "6px 10px", background: colors.white, borderRadius: 8, border: `1px solid ${colors.borderLight}` }}>
                          ...
                          {slotGen.blocks.length > 1 && <button onClick={() => setSlotGen(prev => ({ ...prev, blocks: prev.blocks.filter((_, idx) => idx !== i) }))} style={{ border: "none", background: "none", color: colors.danger, cursor: "pointer", fontSize: 18, padding: 2 }}>×</button>}
                        </div>
                      );
                    })}
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {form.slots.map((slot, i) => (
                <div key={slot.id} style={{ display: "flex", gap: 8, alignItems: "center", padding: "8px 12px", background: colors.bg, borderRadius: 8, border: `1px solid ${colors.borderLight}` }}>
                  ...
                  <select value={slot.type} onChange={e => updateSlot(i, "type", e.target.value)} style={{ padding: "6px 8px", border: `1px solid ${colors.inputBorder}`, borderRadius: 6, fontSize: 13, fontFamily: "inherit", background: colors.white }}>
          ...
          <div style={{ fontSize: 12, color: colors.textMuted, marginTop: -8, marginBottom: 8, paddingLeft: 2 }}>📰 Link to the school's newsletter page. Used in the Interruptions tab to scan for upcoming events.</div>
          ...
          <div style={{ fontSize: 12, color: colors.textMuted, marginTop: -8, marginBottom: 8, paddingLeft: 2 }}>✉ Gmail "Send mail as" alias to use when emailing this school's contacts. Must be configured in your Gmail settings.</div>
          ...
          <div style={{ fontSize: 12, color: colors.textMuted, marginTop: -8, marginBottom: 8, paddingLeft: 2 }}>📤 Endpoint to receive timetable uploads from the Export → Send → Upload to link option.</div>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: colors.text, marginBottom: 6 }}>AI Scanning Guidance</div>
            <textarea value={form.newsletterGuidance || ""} onChange={e => setForm(p => ({ ...p, newsletterGuidance: e.target.value }))} rows={3}
              placeholder={"e.g. \"Follow the link to the latest newsletter PDF\", \"Look for dates in the calendar section at the bottom\"..."}
            ...
            <div style={{ fontSize: 11, color: colors.textMuted, marginTop: 4, paddingLeft: 2 }}>These instructions are automatically sent to the AI when scanning this school's newsletter from the Interruptions tab.</div>
          </div>
          ...
          {schools.map(school => {
            ...
            return (
              <div key={school.id} style={{ borderRadius: 10, overflow: "hidden", border: `1px solid ${colors.borderLight}` }}>
                <div onClick={() => setSchoolOpen(prev => ({ ...prev, [school.id]: !prev[school.id] }))}
                  ...
                  <button onClick={e => { e.stopPropagation(); editSchool(school); }} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", padding: "3px 10px", borderRadius: 6, background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.25)", color: colors.white, cursor: "pointer", fontSize: 12, fontFamily: "inherit", fontWeight: 600 }}>✏️ Edit</button>
                  <button onClick={e => { e.stopPropagation(); deleteSchool(school.id); }} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: 6, background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.25)", color: colors.white, cursor: "pointer", flexShrink: 0 }}>
                ...
                {isOpen && (
                  <div style={{ padding: "12px 14px", background: colors.white }}>
                    <div style={{ fontSize: 13, color: colors.textLight, marginBottom: 6 }}>
                      {school.days.map(d => d.slice(0, 3)).join(", ")} · {school.slots.length} time slots · Specialist scheduling: {school.specialistPolicy === "yes" ? "allowed" : school.specialistPolicy === "no" ? "not allowed" : "allowed, prefer to avoid"}
                    </div>
                    {(school.teacherBreaks || []).length > 0 && (
                      <div style={{ fontSize: 12, color: colors.textMuted, marginBottom: 6 }}>☕ Teacher breaks: {school.teacherBreaks.map(b => `${b.start}–${b.end}`).join(", ")}</div>
                    )}
                    {school.newsletterUrl && <div style={{ fontSize: 12, color: colors.accent, marginBottom: 6 }}>📰 Newsletter: {school.newsletterUrl}</div>}
